...
from openrlhf.trainer.ray import create_vllm_engines, Evaluator2
...
def train(args):
    ...
    # tokenizer = getattr(processor, "tokenizer", processor._tokenizer)
...
if __name__ == "__main__":
    ...
    # Eval subset (useful for quick debugging on small datasets)
    parser.add_argument(
        "--eval_subset_size",
        type=int,
        default=0,
        help="If >0, only evaluate on the first N samples (optionally after shuffling). 0 means full eval set.",
    )
    parser.add_argument(
        "--eval_subset_shuffle",
        action="store_true",
        default=False,
        help="When --eval_subset_size > 0, shuffle the eval dataset before taking the first N samples.",
    )
    parser.add_argument(
        "--eval_subset_seed",
        type=int,
        default=-1,
        help="Seed used for eval subset shuffling. -1 means use --seed.",
    )
    parser.add_argument(
        "--eval_dataloader_shuffle",
        action="store_true",
        default=False,
        help="Shuffle eval dataloader with the training sampler. Useful only for reproducing old runs.",
    )
    ...
    parser.add_argument("--eval_batch_size_pergpu", type=int, default=32)
    ...
    parser.add_argument(
        "--keep_image_placeholder",
        action="store_true",
        default=False,
        help=(
            "If set, keep literal '<image>' placeholders in the final prompt text. "
            "By default we strip '<image>' after converting placeholders to Qwen vision tokens "
            "like '<|vision_start|><|image_pad|><|vision_end|>'."
        ),
    )
