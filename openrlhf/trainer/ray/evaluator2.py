...
from openrlhf.trainer import Evaluator
...
class Evaluator2(Evaluator):
    ...
    def evaluate(self, global_steps):
        ...
        # Optional: shuffle + evaluate only a subset (e.g., quick debug on 50/191 samples)
        subset_size = int(getattr(args, "eval_subset_size", 0) or 0)
        if subset_size > 0:
            subset_seed = getattr(args, "eval_subset_seed", -1)
            subset_seed = args.seed if subset_seed is None or int(subset_seed) < 0 else int(subset_seed)
            do_shuffle = bool(getattr(args, "eval_subset_shuffle", False))
            if do_shuffle:
                try:
                    eval_data = eval_data.shuffle(seed=subset_seed)
                except Exception as e:
                    # Some dataset types may not support shuffle; fall back to deterministic head.
                    strategy.print(f"[Warning] eval_data.shuffle failed ({type(e).__name__}: {e}); using head() without shuffle.")
            try:
                n = min(subset_size, len(eval_data))
                eval_data = eval_data.select(range(n))
            except Exception as e:
                raise RuntimeError(
                    f"Failed to subset eval_data (size={subset_size}, shuffle={do_shuffle}). "
                    f"Underlying type={type(eval_data)} error={type(e).__name__}: {e}"
                )
