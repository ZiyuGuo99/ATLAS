...
try:
    from transformers import Qwen2_5_VLProcessor, Qwen2Tokenizer, Qwen2TokenizerFast
except ImportError as e:
    Qwen2_5_VLProcessor = None
    Qwen2TokenizerFast = None

# Also try to import from the full path (for cases where direct import fails but class exists)
try:
    from transformers.models.qwen2_5_vl.processing_qwen2_5_vl import Qwen2_5_VLProcessor as Qwen2_5_VLProcessor_FullPath
    if Qwen2_5_VLProcessor is None:
        Qwen2_5_VLProcessor = Qwen2_5_VLProcessor_FullPath
except ImportError:
    Qwen2_5_VLProcessor_FullPath = None

# Try to import Qwen2TokenizerFast from full path
try:
    from transformers.models.qwen2.tokenization_qwen2_fast import Qwen2TokenizerFast as Qwen2TokenizerFast_FullPath
    if Qwen2TokenizerFast is None:
        Qwen2TokenizerFast = Qwen2TokenizerFast_FullPath
except ImportError:
    pass

# Try to import Qwen3-VL processor classes
...
# Add Qwen2_5_VLProcessor only if it was successfully imported
if Qwen2_5_VLProcessor is not None:
    DATA_PROCESSOR_MAP[Qwen2_5_VLProcessor] = Qwen2VLDataProcessor
    print(f"✓ Registered Qwen2_5_VLProcessor in DATA_PROCESSOR_MAP")
if "Qwen2_5_VLProcessor_FullPath" in globals() and Qwen2_5_VLProcessor_FullPath is not None and Qwen2_5_VLProcessor_FullPath not in DATA_PROCESSOR_MAP:
    DATA_PROCESSOR_MAP[Qwen2_5_VLProcessor_FullPath] = Qwen2VLDataProcessor
    print("✓ Registered Qwen2_5_VLProcessor_FullPath in DATA_PROCESSOR_MAP")

if Qwen2TokenizerFast is not None:
    DATA_PROCESSOR_MAP[Qwen2TokenizerFast] = Qwen2VLDataProcessor
    print(f"✓ Registered Qwen2TokenizerFast in DATA_PROCESSOR_MAP")

# Add Qwen3-VL processor if available
if Qwen3VLProcessor is not None:
    DATA_PROCESSOR_MAP[Qwen3VLProcessor] = Qwen2VLDataProcessor
    print(f"✓ Registered Qwen3VLProcessor in DATA_PROCESSOR_MAP")
